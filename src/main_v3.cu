...
__global__ sync_kernel(int *dev_mat, int *dev_streak, int *dev_counter, int *prev, int dim, dim3 gridSize , dim3 blockSize) {
    __syncthreads();
...
int main(void){
    ...
    sync_kernel<<<1, 1>>>(dev_mat, dev_streak, dev_counter, prev, n, gridSize, blockSize);
    // }
